...
meta_file = 'checkpoint/sample_submission.csv'
pred_dir = f"results/clef_fungi_results_epoch{epoch}"
out_name = f"results/paddy_rice_epoch{epoch}.csv"
...
history_obs_id = 1
history_score = 0
image_ids = meta.values
with open(out_name, 'w+') as file:
    file.writelines(['image_id', 'label', '\n'])

labels = []
ids = []
for i in range(len(image_ids)):
    csv_id = image_ids[i][0].replace('.jpg', '.csv').replace(',', '')
    csv_id = osp.join(pred_dir, csv_id)
    label = pd.read_csv(csv_id, header=None).values[0][0].split(';')[0]
    with open(out_name, 'a+') as file:
        file.writelines([image_ids[i][0], label, '\n'])
