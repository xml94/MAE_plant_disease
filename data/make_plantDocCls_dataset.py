"""
download dataset from https://github.com/pratikkayal/PlantDoc-Dataset
step 1: change the directory to PlantDoc_cls
step 2: make train, val, and keep the original testing dataset
    setting 1: 20%, 20%
    setting 2: 40%, 20%
    setting 3: 60%, 20%
    setting 4: 80%, 10%
step 3: make train, val, and keep the original testing dataset
    setting 1: 1 shot, 20%
    setting 2: 5 shot, 20%
...
abs_source_dir = "/data/Mingle/DATASETS/PlantDoc_cls"
abs_target_dir = "/data/Mingle/DATASETS_after/PlantDoc_cls/all"
...
# remove the class because it is not in the testing dataset
shutil.rmtree(osp.join(abs_target_dir, 'train', 'Tomato two spotted spider mites leaf'))
# shutil.rmtree(osp.join(abs_target_dir, 'test', 'Tomato two spotted spider mites leaf'))
...
for i in range(len(train_ratio)):
    ...
    src_test_dir = osp.join(abs_source_dir, 'test')
    # tgt_test_dir = osp.join(abs_target_dir)
    list_files = subprocess.run(["cp", "-r", src_test_dir, target_dir])
